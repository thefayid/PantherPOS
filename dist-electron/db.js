    ...
    try {
        ...
        try {
            ...
            if (productCount === 0 && customerCount === 0 && billCount === 0) {
                ...
                const now = new Date();
                const isoDaysAgo = (d) => new Date(now.getTime() - d * 24 * 60 * 60 * 1000).toISOString();
                // Groups
                ...
                for (const p of demoProducts) {
                    db.run(`INSERT INTO products (id, name, barcode, cost_price, sell_price, stock, gst_rate, hsn_code, min_stock_level)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, p);
                }
                // Group associations
                const groupLinks = [
                ...
                for (const c of demoCustomers) {
                    db.run(`INSERT INTO customers (id, name, phone, email, address, created_at, total_purchases, last_visit, points, balance)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, c);
                }
                // Customer ledger for the one customer with balance due
                db.run(`INSERT INTO customer_ledger (customer_id, type, amount, balance_after, description, reference_id, date)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`, [3, "DEBIT", 180, 180, "Previous Due", null, isoDaysAgo(5)]);
                ...
                for (const s of demoSuppliers) {
                    db.run(`INSERT INTO suppliers (id, name, phone, email, address, gstin, created_at, balance)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, s);
                }
                ...
                for (const po of demoPurchaseOrders) {
                    db.run(`INSERT INTO purchase_orders (id, order_no, supplier_id, date, total_amount, status, notes, receive_date)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, po);
                }
                ...
                for (const ct of demoCashTx) {
                    db.run(`INSERT INTO cash_transactions (id, session_id, type, amount, reason, time)
                         VALUES (?, ?, ?, ?, ?, ?)`, ct);
                }
            ...
            if (productCount === 0 && customerCount === 0 && billCount === 0) {
                ...
                try {
                    ...
                    if (promoCount === 0) {
                        const now = new Date();
                        const isoDaysAgo = (d) => new Date(now.getTime() - d * 24 * 60 * 60 * 1000).toISOString();
                        db.run(`INSERT INTO promotions (name, type, code, discount_type, discount_value, min_cart_value, max_discount, start_date, end_date, active, description)
                             VALUES (?, 'COUPON', ?, 'PERCENT', ?, ?, ?, ?, ?, 1, ?)`, ["WELCOME10", "WELCOME10", 10, 199, 100, isoDaysAgo(30), isoDaysAgo(-30), "10% off on bills above ₹199 (max ₹100)"]);
                    }
                }
                catch (e) { }
                // Automation rules
                try {
                    ...
                    if (autoCount === 0) {
                        ...
                        for (const r of rules) {
                            db.run("INSERT INTO automation_rules (name, trigger_type, action_type, config_json, is_active) VALUES (?, ?, ?, ?, ?)", r);
                        }
                    }
                }
                catch (e) { }
                ...
                try {
                    ...
                    if (taskCount === 0) {
                        ...
                        for (const t of tasks) {
                            db.run(`INSERT INTO tasks (title, description, status, priority, due_date, start_date, created_at, updated_at, creator_id, assignee_id, related_entity_type, related_entity_id, tags, is_archived)
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, t);
                        }
                        const firstTask = db.exec("SELECT id FROM tasks ORDER BY id ASC LIMIT 1");
                        const firstId = firstTask?.[0]?.values?.[0]?.[0];
                        if (firstId) {
                            db.run("INSERT INTO task_comments (task_id, user_id, comment, created_at) VALUES (?, ?, ?, ?)", [firstId, 1, "Use Settings → Receipt Template to adjust text size.", isoDaysAgo(0)]);
                            db.run("INSERT INTO task_comments (task_id, user_id, comment, created_at) VALUES (?, ?, ?, ?)", [firstId, 1, "If printer is missing, check USB and restart the app.", isoDaysAgo(0)]);
                        }
                    }
                }
                catch (e) { }
                ...
                try {
                    ...
                    if (chatCount === 0) {
                        ...
                        for (const c of chats) {
                            db.run("INSERT INTO chat_logs (sender, text, action_taken, timestamp) VALUES (?, ?, ?, ?)", c);
                        }
                    }
                }
                catch (e) { }
        ...
        try {
            db.run("ALTER TABLE cash_drawer_sessions ADD COLUMN expected_cash REAL");
        }
        catch (e) { }
        try {
            db.run("ALTER TABLE cash_drawer_sessions ADD COLUMN variance REAL");
        }
        catch (e) { }
        try {
            db.run("ALTER TABLE cash_transactions ADD COLUMN meta_json TEXT");
        }
        catch (e) { }
        try {
            db.run("CREATE INDEX IF NOT EXISTS idx_cash_transactions_session_id ON cash_transactions(session_id)");
        }
        catch (e) { }
